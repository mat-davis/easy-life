#AWK to parse Checkpoint fwlog for accept entries then display source IP, destination IP, protocol & port (no headers)
...
/Built/ {
 if (($8 ~ /^TCP$/ || $8 ~ /^UDP$/) && $7 ~ /^inbound$/) printf "%-16s %-16s %-8s %-5s\n" ,substr($13,1+ index($13,"("),-2 + index($13,"/")),substr($16,1+ index($16,"("),-2 + index($16,"/")),$8,substr($15,1+index($15,"/"),5)
 }
/Built/ {
